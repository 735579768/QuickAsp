...
'输出栏目和子菜单
	...
	'查一级菜单
	set rs=olddb.query("select sysmenuid,menu_name,parent_menu_id,menu_link,sort from " & suffix & "sysmenus  where parent_menu_id=0 order by sort asc ")
	do while not rs.eof
			'查询二级菜单
			set rss=olddb.query("select sysmenuid,menu_name,menu_link,sort from " & suffix & "sysmenus  where parent_menu_id="&rs("sysmenuid")&" order by sort asc ")
